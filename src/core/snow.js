function createSnowflake() {
    ...
    var e = Math.floor(Math.random() * 10) + 5,
        t = Math.floor(Math.random() * 90) + 4,
        n = (Math.floor(Math.random() * 100) + 1) / 100,
        snowflakeWrapper = document.createElement("div");

    snowflakeWrapper.className = "flake-wrapper";
    snowflakeWrapper.style.width = "24px";
    snowflakeWrapper.style.left = t + "%";
    snowflakeWrapper.style.height = "40px";
    snowflakeWrapper.style.transform = "scale(" + n + ")";
    snowflakeWrapper.style.animation = "falling " + e + "s linear infinite";
    snowflakeWrapper.innerHTML = flake;

    document.querySelector("body").appendChild(snowflakeWrapper);
}

function removeSnowflake() {
    var snowflakes = document.querySelectorAll(".flake-wrapper");

    snowflakes.forEach(function(snowflake) {
        var offsetTop = snowflake.offsetTop,
        documentHeight = document.documentElement.clientHeight - 100;

        if (offsetTop > documentHeight) {
        snowflake.remove();
        }
    });
}

function initSnowfall(elem) {
    document.addEventListener("DOMContentLoaded", function() {
        setInterval(function() {
            createSnowflake(elem);
            removeSnowflake();
        }, 200);
    });
}
