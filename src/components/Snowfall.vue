  ...
  export default {
    ...
    methods: {
      createSnowFlake() {
        ...
        const t = Math.floor(Math.random() * 90) + 4;
        const n = (Math.floor(Math.random() * 100) + 1) / 100;
        const snowFlake = `<div class='flake-wrapper' style='width:24px; left:${t}%;height:40px;transform:scale(${n});animation:falling ${e}s linear infinite'>${this.flake}</div>`;
        this.$el.insertAdjacentHTML('beforeend', snowFlake);
      },
      removeSnowFlake() {
        const snowFlakeWrappers = document.querySelectorAll('.flake-wrapper');
        snowFlakeWrappers.forEach((snowFlakeWrapper) => {
          const offsetTop = snowFlakeWrapper.offsetTop;
          const documentHeight = document.documentElement.clientHeight;
          if (offsetTop > documentHeight - 100) {
